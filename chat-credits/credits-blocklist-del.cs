using System;
using System.IO;
using System.Collections.Generic;

public class CPHInline
{
    public bool Execute()
    {
        CPH.TryGetArg("rawInput", out string input);
        CPH.TryGetArg("command", out string commandName);

        input = input?.Trim().TrimStart('@');

        if (string.IsNullOrEmpty(input))
        {
            CPH.SendMessage($"Usage: {commandName} <username>");
            return false;
        }

        string logDir = CPH.GetGlobalVar<string>("creditsDir", true);
        if (string.IsNullOrEmpty(logDir))
        {
            CPH.SendMessage("Credits directory is not set.");
            return false;
        }

        string blocklistPath = Path.Combine(logDir, "credits_blocklist.txt");

        if (!File.Exists(blocklistPath))
        {
            CPH.SendMessage("Blocklist file not found.");
            return false;
        }

        try
        {
            List<string> blocklist = new List<string>(File.ReadAllLines(blocklistPath));
            int removed = blocklist.RemoveAll(u => u.Equals(input, StringComparison.OrdinalIgnoreCase));

            if (removed == 0)
            {
                CPH.SendMessage($"{input} was not found on the credits blocklist.");
                return true;
            }

            File.WriteAllLines(blocklistPath, blocklist);
            CPH.SendMessage($"{input} removed from the credits blocklist.");
            return true;
        }
        catch (Exception ex)
        {
            CPH.SendMessage($"Failed to update blocklist: {ex.Message}");
            return false;
        }
    }
}